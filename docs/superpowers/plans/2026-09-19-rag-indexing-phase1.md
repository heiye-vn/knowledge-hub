...
- [x] **步骤 1：实现百炼兼容客户端**
  ...
  `OpenAIEmbeddings` + `baseURL = EMBEDDING_BASE_URL`，`model = qwen3.7-text-embedding-flash`，`dimensions = 1024`，`stripNewLines: false`。
  （原计划 `text-embedding-v3`，因账号无可用额度于 2026-09-19 改用 flash；两者默认维度同为 1024，索引结构不变。）
- [x] **步骤 2：batch 钳制**
