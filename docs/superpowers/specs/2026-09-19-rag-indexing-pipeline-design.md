...
| `RAG_MIN_SCORE` | `0.72` | 向量召回最低分，低于此值丢弃。**换模型/换语料必须重新校准** |
...
| **相似度阈值区分度有限**（实测） | 相关查询 0.7478~0.8050，不相关 0.6519~0.7081，**区间接近甚至可能重叠**；口语化查询（如「东西误删了还能找回来吗」）相似度仅 0.62 且会召回错文档 | 阶段三引入 **Rerank 模型**（qwen3-rerank / bge-reranker）做精排，或用 LLM 做查询改写；`RAG_MIN_SCORE` 只是止血手段 |
| **BM25 侧无阈值** | 关键词路没有最低分门槛，词项偶然匹配（如「工资」→「薪资」）仍会召回 | 必要时加 `RAG_MIN_KEYWORD_SCORE`；BM25 分数无归一化上界，需单独校准 |
...
#### 实测：向量召回必须设最低分门槛（2026-09-19）

用真实 API 跑 3 篇中文制度文档后发现一个**必须修的缺陷**：
kNN 是「找最近的 K 个」，**永远会返回 topK 条**，即使查询与知识库完全无关也会硬凑结果。
例如「手头紧想提前支取工资」「今天天气怎么样」都返回了 3 条，相似度 0.65~0.71 ——
这种噪声喂给 LLM 会直接导致幻觉。（BM25 无此问题：无词项匹配就不返回。）

为此新增 `RAG_MIN_SCORE`。阈值**不能拍脑袋**，实测分位如下
（注意：ES 的 cosine `_score` 是 `(1+cos)/2` 映射值，非原始余弦）：
...
| 口令多久要换一次 | 0.7722 | 相关 |
| 数据备份多久做一次 | 0.7478 | 相关 |
| 手头紧想提前支取工资 | 0.7081 | **不相关** |
| 今天天气怎么样 | 0.6519 | **不相关** |

分界落在 0.7478 与 0.7081 之间 → 取 **0.72**（偏保守：宁可漏召也不误召）。
曾先试 0.75，结果把 0.7478 的正确结果误杀，故下调。

**局限**：相关与不相关区间接近，单靠阈值无法彻底分开；
且口语化查询（「东西误删了还能找回来吗」0.6247）会召回错文档。
根治要靠阶段三的 Rerank 或查询改写，`RAG_MIN_SCORE` 只是止血。
