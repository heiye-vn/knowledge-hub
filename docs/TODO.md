...
> 记录时间：2026-09-24（四状态流转 + 审核流水实现完成时）
> 状态流转本身已闭环（草稿 → 待审核 → 已发布 → 归档 / 下架），下表是**明确推迟**的事项。

| 项 | 说明 | 触发条件 |
| :--- | :--- | :--- |
| ~~**审核人身份与权限**~~ | ~~`reviewerId` / `reviewerName` 由请求体传入，且任何调用方都能 approve / reject~~ | ✅ **已解决（2026-09-25，feat-v7）**：审核人改从 JWT 登录态取（`@CurrentUser()`），`ReviewDecisionDto` 两字段删除；审核工作台 4 端点加 `@Roles(ROLE_REVIEWER, ROLE_ADMIN)`，其余调用方返回 403 |
| **审核撤回（withdraw）** | 作者提交后只能等审核员处理，无法自行撤回 | 出现「提错了要撤」的实际反馈时加，语义同 reject 但由作者发起、不产生审核结论 |
