...
### 示例二：技术架构类（草稿 + 全局公开 + 携带封面图）

```bash
curl -s -X POST http://localhost:3000/documents \
  -H 'Content-Type: application/json' \
  -d '{
    "title": "企业级 RAG 知识检索增强架构设计方案",
    "content": "# 企业级 RAG 知识检索增强架构设计方案\n\n## 1. 背景与目标\n\n随着企业知识库规模扩展至十万篇以上，传统关键字全文检索在语义理解、跨文档聚合问答场景下效果瓶颈明显。本项目旨在搭建基于 pgvector + BGE 向量模型的高性能 RAG（Retrieval-Augmented Generation）知识引擎。\n\n## 2. 核心架构设计\n\n整体架构分为三层：\n\n### 2.1 摄取与切分管道（Ingestion Pipeline）\n- **文档解析**：支持 Markdown、PDF、Word 多格式自动化抽取。\n- **语义分块（Chunking）**：采用递归字符切分器（RecursiveCharacterTextSplitter），Chunk Size 设为 500 字符，Overlap 设为 50 字符。\n- **向量嵌入（Embedding）**：调用 `BAAI/bge-large-zh-v1.5` 模型输出 1024 维向量。\n\n### 2.2 混合检索层（Hybrid Search）\n- **Dense Retrieval**：PostgreSQL pgvector HNSW 索引（余弦距离）。\n- **Sparse Retrieval**：PostgreSQL tsvector 全文倒排索引（BM25 算法）。\n- **Reranking**：基于 `bge-reranker-large` 对 Top 50 候选集进行二次交叉打分，截取 Top 5。\n\n### 2.3 生成与提示工程（Generation）\n- 结合召回上下文、来源文档溯源引用（Citation），流式注入大模型（DeepSeek / Qwen）完成严谨回答。\n\n## 3. 性能基准指标\n\n| 指标项 | 目标 SLA | 当前压测表现 |\n| :--- | :--- | :--- |\n| 向量检索耗时（P95） | ≤ 30ms | 18ms |\n| 端到端首字响应（TTFT） | ≤ 800ms | 620ms |\n| 知识召回准确率（Hit@5）| ≥ 88% | 91.4% |\n\n---\n\n审核人：架构委员会｜状态：技术方案评审中",
    "summary": "针对知识库多源文档解析、pgvector 混合检索与大模型生成的企业级 RAG 架构设计规范与选型评估。",
    "tags": "AI,RAG,向量检索,LLM,架构设计,pgvector",
    "status": 0,
    "isPublic": true,
    "coverImage": "https://img.company.internal/covers/rag-arch-v1.png",
    "remark": "待 Q3 架构委员会评审通过后发布",
    "categoryId": "20002",
    "teamId": "30002",
    "authorId": "10002",
    "createBy": "10002"
...
创建成功后，把返回的 `id`（位于 `data.id`）赋给变量：

```bash
DOC_ID='替换成返回的 data.id'
# 若在 shell 中配合 jq 自动提取：
# DOC_ID=$(curl -s -X POST 'http://localhost:3000/documents' ... | jq -r '.data.id')
```
