...
### 5.1 提交信息禁写内容（强制，2026-09-23 起）

提交信息**只描述本次改动本身**，禁止出现下列内容（标题与正文均适用）：

- 任何**外部来源指涉**：如「参考项目」「对照某仓库」「按 vN 对齐」「沿用某实现」等
- **章节 / 教程 / 课程**类字样：如「第 N 章」「第 N 讲」「跟做」「教程」等
- 内部对照文档的具体文件名（可用「差异登记表」等中性说法指代）

原因：提交历史是对外可见的，这类措辞会让项目看起来是跟随式学习产物，
而非独立设计与决策的结果（用户明确要求）。

**技术理由照常保留**——写清「为什么这么做」是允许的，只是不提「参照了谁」：

- ❌ `fix(document): 对照参考项目 v3 时发现缺失状态校验，故按 v3 对齐`
- ✅ `fix(document): 补齐发布状态守卫，禁止归档文档重新发布`
- ❌ `docs: 更新笔记与参考项目对齐映射`
- ✅ `docs: 更新模块笔记与差异登记`

代码注释同此规范：用「基线实现」等中性表述替代具体出处，保留分叉与修复的技术理由。
